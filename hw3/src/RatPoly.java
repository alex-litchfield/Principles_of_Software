...
public final class RatPoly {
    ...
    public RatPoly negate() {
        if (this.isNaN() ) {
            ...
            RatNum[] newRatPoly = new RatNum[coeffs.length];
            for(int i = 0; i < coeffs.length; i++)
                newRatPoly[i] = coeffs[i].negate();
            return new RatPoly(newRatPoly);
        }
    }
    ...
    public RatPoly add(RatPoly p) {
        if (this.isNaN() || p.isNaN() ) {
        ...
        else if (p.equals(ZERO) && this.equals(ZERO) ) {
            return ZERO;
            ...
            {
                rats = new RatNum[p.coeffs.length];
                for(int i = 0; i < rats.length; i++)
                    rats[i] = p.coeffs[i];
                for(int i = 0; i < this.coeffs.length; i++)
                    rats[i] = rats[i].add(this.coeffs[i]);
            }
            else {
                rats = new RatNum[this.coeffs.length];
                for(int i = 0; i < rats.length; i++)
                    rats[i] = this.coeffs[i];
                for(int i = 0; i < p.coeffs.length; i++)
                    rats[i] = rats[i].add(p.coeffs[i]);
                if( (this.degree == p.degree) && (rats[rats.length - 1].equals(new RatNum(0) ) ) ){
                    return ZERO;
                }
            }
    ...
    public RatPoly mul(RatPoly p) {
        if (this.isNaN() || p.isNaN() ) {
            ...
            RatNum[] rats = new RatNum[coeffs.length + p.coeffs.length - 1];
            for (int i =0; i < rats.length; i++) {
                rats[i] = new RatNum(0);
            }
            for (int i = 0; i < this.coeffs.length; i++) {
                for(int j = 0; j < p.coeffs.length; j++) {
                    rats[i + j] = rats[i + j].add(this.coeffs[i].mul(p.coeffs[j]));
                }
            }
            if (rats[rats.length - 1].equals(new RatNum(0 ) ) ) {
                return ZERO;
            } else {
                return new RatPoly(rats);
            }
    ...
    public RatPoly div(RatPoly p) {
        if (this.isNaN() || p.isNaN() || p.equals(ZERO) ) {
    		...
    		RatNum [] q = new RatNum[this.coeffs.length + 1 - p.coeffs.length];
    		RatNum [] r = new RatNum[this.coeffs.length];
    		for(int i = 0; i < r.length; i++) {
                r[i] = this.coeffs[i];
            }

    		int k = this.coeffs.length - p.coeffs.length;
    		int j = 1;
    		while (r.length >= p.coeffs.length && r.length > 0 && k >= 0) {
    			RatNum t = r[r.length - j].div(p.coeffs[p.coeffs.length - 1] );
    			q[k] = t;
                j = j + 1;
                k = k - 1;
    			for (int i = r.length - p.coeffs.length - j + 2; i < r.length - j + 2; i++) {
                    r[i] = r[i].sub(t.mul(p.coeffs[p.coeffs.length + i + j - r.length - 2] ) );
                }
    		}
    ...
    public double eval(double d) {
        if (this.isNaN() ) {
            ...
            double finalVal = 0;
            for (int i = 0; i < coeffs.length; i++) {
                finalVal = finalVal + Math.pow(d, i) * coeffs[i].doubleValue();
            }
            return finalVal;
        }
    }  
    ...
    public RatPoly differentiate() {
        if (this.isNaN() ) {
            ...
            RatNum[] rats = new RatNum[this.coeffs.length - 1];
            for (int i = 0; i < rats.length; i++) {
                rats[i] = this.coeffs[i + 1].mul(new RatNum(i + 1));
            }
            return new RatPoly(rats);
        }
    }
    ...
    public RatPoly antiDifferentiate(RatNum integrationConstant) {
        if (this.isNaN() || integrationConstant.isNaN() ) {
            ...
            for (int i = 1; i <= rats.length - 1; i++) {
                rats[i] = this.coeffs[i - 1].div(new RatNum(i));
            }
            if (rats[rats.length - 1].equals(new RatNum(0) ) )
                return ZERO;
            return new RatPoly(rats);
    ...
    public double integrate(double lowerBound, double upperBound) {
        if (this.isNaN() || Double.isNaN(lowerBound) || Double.isNaN(upperBound) ) {
            ...
            RatPoly finalVal = antiDifferentiate(new RatNum(1));
            return finalVal.eval(upperBound) - finalVal.eval(lowerBound);
        }
    }
