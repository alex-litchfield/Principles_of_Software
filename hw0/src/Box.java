...
class ballSort implements Comparator<Ball> {

    public int compare(Ball alpha, Ball omega) {
        return Double.compare(alpha.getVolume(), omega.getVolume());
        //return alpha.getVolume().Double.compareTo(omega.getVolume());
    }
...
public class Box implements Iterable<Ball> {
    ...
    public boolean add(Ball b) {
        ...
        for (Ball ball : ballContainer) {
            currentVol = currentVol + ball.getVolume();
        }
        if (!ballContainer.contains(b)) {
            if ((b.getVolume() + currentVol) <= maxVol) {
                ballContainer.add(b);
                return true;
            }
        }
        ...
        //return ballContainer.add(b) && (b.getVolume() + currentVol) <= maxVol;
    }
